     ...
     <div class="main-panel">
                    ...
                    <table class="table table-bordered">
                      <thead>
                        <tr>
                          ...
                          <th>
                             Valor total das receitas nesta conta
                          </th> 
                          <th>
                             Receitas nessa conta
                          </th> 
                          <th style="width:0">
                            Ações
                          </th> 
                        ...
                        <tr >
                          <td>
                           <?php if(strlen($value->name) > 40) {
                            ...
                            <td>
                            <?php $value_new = number_format($value->sum_receitas, 2, ',', '.');  ?>
                            R$ {{$value_new}}
                          </td>
                          <td>
                            {{$value->count_receitas}}
                          </td> 
                          <td >
                            <a href="/conta/edit/{{$value->id}}"><button style="border-radius: 2px" class="btn btn-sm btn-primary" >Editar</button></a>

                            <a onclick="setDel({{$value->id}}, {{$value->count_receitas}})" data-bs-toggle="modal" data-bs-target="#exampleModal" href="/trash/{{ $value->id }}?link={{url()->full()}}"><button style="border-radius: 2px" class="btn btn-sm btn-danger">Excluir</button></a>
                          </td>
                        </tr>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Deseja mesmo excluir essa conta?</h5>
        ...
        <script>
           ...
           function setDel (id, countFinancas) {
            document.getElementById('actionUrl').href = `/del-conta/${id}`;

            if(countFinancas > 0) {
              document.getElementById('actionUrl').style.display = 'none';

              document.getElementById('modalText').style.display = 'block';
              document.getElementById('modalText').innerHTML = 'Para excluir essa conta primeiro é necessário que você a remova de todas as receitas associdas a ela. <span style="margin-top:5px;display:block"> Existe '+countFinancas+' receita(s) associada(s) a esta conta </span>';
